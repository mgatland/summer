...
for (var x = 0; x < gridXLength; x++) {
	...
	for (var y = 0; y < gridYLength; y++) {
		roomxy[x].push({x:x,y:y,color:Math.random() > 0.5 ? "#FFAA9B" : "#FFB070"})
...
var features = {
	...
	blocker: {sprite: blockerSprite, produces: 1, delay:4, name:"scorch"}
}
...
function playerAction(action) {
	if (state === "waiting") {
		if (action === "block") {
			blockers--
			scorch(selected)
			rooms.filter(r => adjacent(selected, r)).forEach(room => scorch(room))
			nextDay()
		}
...
function scorch(room) {
	room.scorched = 3
	delete room.dirt
	delete room.feature
	delete room.ants
	ants = ants.filter(r => r.x != room.x || r.y != room.y)
...
function nextDay() {
	...
	if (food < 0) {
		...
		state = "end"
		endMessage = "You survived summer!"
	}
}
...
function diagonalAdjacent(one, two) {
	return (adjacent(one, two) ||
		(one.x === two.x - 1 && (one.y === two.y - 1 || one.y === two.y + 1))
		|| 
		(one.x === two.x + 1 && (one.y === two.y - 1 || one.y === two.y + 1))
		)
}
...
function draw() {
	...
	for (var x = 0; x < gridXLength; x++) {
		for (var y = 0; y < gridYLength; y++) {
			...
			for (let i = 0; i < room.scorched; i++) {
				ctx.fillStyle = 'rgba(60, 20, 20, 0.2)'
				ctx.fillRect(x*cellWidth, y*cellHeight, cellWidth, cellHeight)
			}
			ctx.strokeStyle = "#eee"
			ctx.lineWidth = 2
	...
	ctx.textAlign = "left"
	ctx.fillStyle = "white"
	ctx.fillText(daysLeft + " days left. " + food + " food, " + blockers + " scorchers.", 10, height - 130)
	...
	if (selected === undefined) {
		...
		if (selected.blocked || selected.ants) {
			if (!canPlayerMove()) {
				...
				buttons[0].line1 = "can't work"
				buttons[0].line2 = "here"
				buttons[0].action = "no op"
				drawButton(buttons[0])
			}
		} else {
			if (blockers > 0) {
				buttons[0].line1 = "Scorch"
